...
for (i=0; i<dragoLength/3; i++) {
  bodyNodes.push(dragBody.mk('sphere', {
    position:`0 0 -${i/2}`, radius:.5, color
  }));
}
var z = (dragoLength/3)/2;
for (i=1; i<=dragoLength*2/3; i++) {
  let r = .5 - i/(dragoLength*1.8);
  bodyNodes.push(dragBody.mk('sphere', {
...
lastNode.mk('cone', {
  position: '0 0 .1', rotation: '90 0 0', scale:'1 1 .1',
  'radius-bottom': .2, 'radius-top': 0, height: .4, color
});
lastNode.mk('cone', {
  position: '0 0 -.3', rotation: '-90 0 0', scale:'1 1 .1',
  'radius-bottom': .2, 'radius-top': 0, height: .4, color
});

var wingLeft = wingNode.mk('cone', {
  position: '.7 0 0', rotation: '0 0 90', scale:'.1 1 1',
  'radius-bottom': .8, 'radius-top': 0, height: .8, color
});
wingLeft.mk('cone', {
  position: '0 -.6 0', rotation: '0 0 180', scale:'1 1 1',
  'radius-bottom': .8, 'radius-top': 0, height: .4, color
});
...
window.dragoTic = function (ticCount) {
  // move it:
  // angSpeed = .015/fps =~ 7.5m/s in a 500m radius circle path.
  angPos += .015/fps;
  ...
  bodyNodes.forEach((node, i)=> {
    node.object3D.position.y = nodeWaveY(i, ticCount) * .3 //* (dragoLength-i+2)/(dragoLength*2);
  });
  dragoHead.object3D.position.y = nodeWaveY(-1, ticCount) * .4;
  dragoHead.object3D.rotation.x = -nodeWaveY(0, ticCount, cos) * .3;
  lastNode.object3D.rotation.x = -nodeWaveY(dragoLength-1, ticCount, cos) * .5;
  var wingWave = -nodeWaveY(1, ticCount, cos);
  wingLeft.object3D.rotation.z = deg90 + wingWave*.8;
  wingLeft.object3D.position.y = wingWave*.2;
  wingRight.object3D.rotation.z = -deg90 - wingWave*.8;
  wingRight.object3D.position.y = wingWave*.2;
